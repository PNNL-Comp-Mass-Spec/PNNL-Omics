...
namespace PNNLOmics.Algorithms.FeatureClustering
{
        ...
        where U : FeatureLight, IFeatureCluster<T>, new()
	{
			...
			for (int i = 0; i < totalFeatures - 1; i++)
			{
				...
				if (ppm > massTolerance)
				{
					...
					else
					{
						...
						List<U> blockClusters                   = AverageLinkUMCs(distances, localClusters);
		...
		private List<U> AverageLinkUMCs(List<PairwiseDistance<T>> distances, Dictionary<int, U> clusters)
		{
			/*
