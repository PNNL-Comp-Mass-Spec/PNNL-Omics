...
namespace LCMS.Regression
{
    class CentralRegression
    {
        ...
        public List<RegressionPts> Points()
        {
            return m_pts;
        }
        ...
        public void CalculateMinMax()
        {
            ...
            m_maxY = -1 * double.MaxValue;

            for (int i = 0; i < numPts; i++)
            {
                RegressionPts point = m_pts[i];
                if (point.X < m_minX)
                {
                    m_minX = point.X;
                }
        ...
        public void CalculateSectionStdAndCount(int interval_num)
        {
            ...
            {
                RegressionPts pt = new RegressionPts();
                pt = m_pts[pt_num];
            ...
            {
                m_normUnifEM.CalculateDistributions(ref points);
        ...
        public void RemoveOutliersAndCopy(ref List<RegressionPts> calibMatches)
        {
            foreach (RegressionPts point in calibMatches)
            {
                m_pts.Add(point);
            }
        ...
        public void CalculateRegressionFunction(ref List<RegressionPts> calibMatches)
        {
            Clear();
        ...
        {
            double x_interval_size = (m_maxX - m_minX) / m_num_x_bins;
            List<RegressionPts> tempPts = new List<RegressionPts>();
            int numPts = m_pts.Count;

            for (int pointNum = 0; pointNum < numPts; pointNum++)
            {
                RegressionPts point = new RegressionPts();
            ...
            foreach (RegressionPts point in tempPts)
            {
                m_pts.Add(point);
            }
        }
